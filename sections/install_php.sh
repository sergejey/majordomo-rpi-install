...
if [ ! -f /var/www/html/php.php ]; then
 ...
 sudo chown www-data:www-data /var/www/html/php.php
fi
...
sudo sed -i 's/error_reporting = E_ALL & ~E_DEPRECATED/error_reporting = E_ALL & ~E_NOTICE & ~E_DEPRECATED/g' /etc/php/7.4/apache2/php.ini

sudo sed -i 's/display_errors = off/display_errors = on/g' /etc/php/7.4/cli/php.ini
sudo sed -i 's/error_reporting = E_ALL & ~E_DEPRECATED/error_reporting = E_ALL & ~E_NOTICE & ~E_DEPRECATED/g' /etc/php/7.4/cli/php.ini
